<template>
        ...
        <section class="w-full flex flex-col items-center
            ...
            <header ref="heroEl" class="w-full lg:w-3/5 mx-auto flex gap-8
                ...
                <div ref="badgesEl" class="w-1/2 flex flex-col gap-3 pt-6 items-end justify-end
                    ...
                    <span v-for="bullet in other.bullets" :key="bullet" class="bg-stone-800 whitespace-nowrap rounded-full
                       text-lg px-5 py-2
        ...
        <section ref="ctaEl" class="w-full flex justify-center items-center mb-36
            ...
            <div class="w-full lg:w-3/5 mx-auto flex items-end justify-center gap-8
                ...
                <div ref="imageEl" class="w-full aspect-video">
                    <img :src="other.image" class="w-full h-full object-cover rounded-4xl" :alt="other.title" />
                </div>
    ...
    <section ref="relatedEl" id="related-services" :class="sectionPad" class="py-20 mb-32 flex flex-col gap-8
        ...
        <!-- Services cards (qui non serve filtrare per slug: other != services) -->
...
/* padding laterale identico alla tua versione "perfetta" */
...
const other = computed(() => otherServices.value.find((p) => p.slug === route.params.slug) || null);

useSeo(() => ({
    title: `${other.value?.title || "Consulenza digitale"} | KLYK Studio`,
    description: `${other.value?.description || "Consulenza e supporto digitale continuativo per aziende, professionisti, startup e PMI."}`.slice(0, 155),
    path: localizedPath(locale.value, "otherServices", { slug: route.params.slug }),
    lang: locale.value,
    image: other.value?.image,
    alternates: [
        { hreflang: "it", path: localizedPath("it", "otherServices", { slug: route.params.slug }) },
        { hreflang: "en", path: localizedPath("en", "otherServices", { slug: route.params.slug }) },
        { hreflang: "x-default", path: localizedPath("it", "otherServices", { slug: route.params.slug }) },
    ],
    schemas: [
        {
            "@context": "https://schema.org",
            "@type": "Service",
            name: other.value?.title || "Consulenza digitale",
            ...
            url: localizedPath(locale.value, "otherServices", { slug: route.params.slug }),
        },
        breadcrumbSchema([
            { name: t("common.home"), path: localizedPath(locale.value, "home") },
            { name: t("common.services"), path: localizedPath(locale.value, "home") },
            { name: other.value?.title || "Servizio", path: localizedPath(locale.value, "otherServices", { slug: route.params.slug }) },
        ]),
    ],
}));

/* related */
const relatedServices = computed(() => services.value.slice(0, 3));

const relatedOtherServices = computed(() => {
    const currentSlug = other.value?.slug;
    return otherServices.value.filter((p) => p.slug !== currentSlug).slice(0, 2);
});
