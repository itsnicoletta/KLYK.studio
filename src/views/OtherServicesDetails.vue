...
/* data */
const other = computed(() => otherServices.find((p) => p.slug === route.params.slug) || null);

/* related */
const relatedServices = computed(() => services.slice(0, 3));

const relatedOtherServices = computed(() => {
    const currentSlug = other.value?.slug;
    return otherServices.filter((p) => p.slug !== currentSlug).slice(0, 2);
});
